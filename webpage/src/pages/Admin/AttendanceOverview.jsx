  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Attendance by Event</h1>
      </div>
          ...
          <table className="w-full table-auto border-collapse mb-4">
            <thead>
              <tr className="bg-gray-100">
                {["Event", "Date", "School", "Photographer", "Action"].map(h => (
                ...
                <tr key={ev.id}>
                  ...
                  <td className="p-2 border">
                    <button
                      onClick={() =>
                        navigate(
                          routes.ADMIN_ATTENDANCE_DETAILS.replace(":eventId", ev.id)
                        )
                      }
                      className="flex items-center text-blue-600 hover:underline"
                    >
                      <HiClipboardList className="mr-1" /> View Attendance
                    </button>
                  </td>
                </tr>
              ))}
